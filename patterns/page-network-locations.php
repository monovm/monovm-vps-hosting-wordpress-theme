<?php
/**
 * Title: Network and locations page
 * Slug: monovm-blueprint/page-network-locations
 * Categories: monovm-blueprint, services
 * Description: A network and data-center starter page with regions, proof points, security, FAQs, and support paths.
 * Viewport Width: 1440
 * Keywords: network, locations, data center, regions
...
<div class="wp-block-group alignfull monovm-blueprint-section has-canvas-background-color has-background" style="padding-top:var(--wp--preset--spacing--90);padding-right:var(--wp--preset--spacing--70);padding-bottom:var(--wp--preset--spacing--90);padding-left:var(--wp--preset--spacing--70)"><!-- wp:paragraph {"textColor":"signal","fontSize":"small","fontFamily":"technical"} --><p class="has-signal-color has-text-color has-technical-font-family has-small-font-size"><?php echo esc_html__( 'INFRASTRUCTURE / NETWORK', 'monovm-blueprint' ); ?></p><!-- /wp:paragraph --><!-- wp:heading {"level":2,"fontSize":"3-x-large"} --><h2 class="wp-block-heading has-3-x-large-font-size"><?php echo esc_html__( 'Present regional infrastructure with useful context.', 'monovm-blueprint' ); ?></h2><!-- /wp:heading --><!-- wp:paragraph {"textColor":"muted","fontSize":"large"} --><p class="has-muted-color has-text-color has-large-font-size"><?php echo esc_html__( 'Connect locations, network characteristics, operational safeguards, and support information without relying on a remote map.', 'monovm-blueprint' ); ?></p><!-- /wp:paragraph --></div>
<!-- /wp:group -->
